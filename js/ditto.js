...
var duoshuo = '<h2>留言</h2>'+'<div id="comments"><div></div> </div>'
...
function init_edit_button() {
  if (ditto.base_url === null) {
    ...
    $(ditto.edit_id).on("click", function() {
      ...
      window.open(ditto.base_url+'/blob/master'+ hash + ".md");
...
function router() { 
  ...
  $.get(path, function(data) {
    ...
    $(ditto.content_id).html(marked(data)+ duoshuo);
    ...
    sub_div.attr('data-thread-key', path===''?hashcode('README'):hashcode(path)+'' );
